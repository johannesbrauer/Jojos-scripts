...
# Hardlinks (used by Radarr/Sonarr for instant "rename, not copy") require a POSIX
# filesystem. Fail fast on CIFS/SMB NAS mounts that silently break them.
echo "==> Checking that the NAS mount supports hardlinks..."
touch "$NAS_PATH/.hltest" || { echo "ERROR: Cannot write to $NAS_PATH." >&2; exit 1; }
ln "$NAS_PATH/.hltest" "$NAS_PATH/.hltest2" 2>/dev/null \
  || { echo "ERROR: $NAS_PATH does not support hardlinks (CIFS/SMB mount?). Hardlinks are required for the Radarr/Sonarr download handling - use NFS or a POSIX-capable filesystem instead." >&2; rm -f "$NAS_PATH/.hltest"; exit 1; }
rm -f "$NAS_PATH/.hltest" "$NAS_PATH/.hltest2"
...
ip link set veth-ns netns ${NS_NAME} 2>/dev/null || true
...
# Host forwarding must be on BEFORE the handshake - the namespace reaches the provider's Endpoint only by hopping through the host.
sysctl -w net.ipv4.ip_forward=1 >/dev/null

# Resolve the Endpoint HERE at runtime (not baked at install time) so providers
# using a hostname keep working even if the IP changes.
WG_ENDPOINT_IP=\$(grep -iE '^\s*Endpoint\s*=' /etc/wireguard/wg0.conf | head -n1 | sed -E 's/^[[:space:]]*[Ee]ndpoint[[:space:]]*=[[:space:]]*//; s/[[:space:]]*$//')
WG_ENDPOINT_IP=\${WG_ENDPOINT_IP%:*}
WG_ENDPOINT_IP=\${WG_ENDPOINT_IP#[}
WG_ENDPOINT_IP=\${WG_ENDPOINT_IP%]}
if [[ ! "\${WG_ENDPOINT_IP}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\$ ]]; then
  if [[ "\${WG_ENDPOINT_IP}" =~ ^[0-9a-fA-F:]+$ ]]; then
    echo "ERROR: IPv6 WireGuard Endpoints are not supported." >&2; exit 1
  fi
  WG_ENDPOINT_IP=\$(getent ahostsv4 "\${WG_ENDPOINT_IP}" | awk 'NR==1{print \$1}' || true)
  [[ -n "\${WG_ENDPOINT_IP}" ]] || { echo "ERROR: Cannot resolve WireGuard Endpoint." >&2; exit 1; }
fi

# Remember the exact IP up.sh resolved so down.sh can delete precisely the rules
# we created (a hostname in the wg0.conf Endpoint would otherwise never match).
echo "\${WG_ENDPOINT_IP}" > /run/vpn-netns.endpoint

# Bootstrap host-route: ONLY the VPN Endpoint IP may leave the namespace via the host (handshake packets).
# No default route through the host exists in this namespace - that is exactly what keeps it leak-proof if the tunnel drops.
ip netns exec ${NS_NAME} ip route replace \${WG_ENDPOINT_IP}/32 via ${VETH_HOST_IP%/*} dev veth-ns

# The namespace's source address is private and unroutable outside this host, so
# the tunnel carrier (encrypted) packets must be masqueraded behind the host's own IP - otherwise the VPN server's replies never find their way back.
# Scoped strictly to the Endpoint IP: nothing else can ever be masqueraded out.
iptables -t nat -D POSTROUTING -s ${VETH_NS_IP} -d \${WG_ENDPOINT_IP} -j MASQUERADE 2>/dev/null || true
iptables -t nat -A POSTROUTING -s ${VETH_NS_IP} -d \${WG_ENDPOINT_IP} -j MASQUERADE

# Explicitly allow the carrier traffic out, in case the host's FORWARD policy is DROP.
iptables -D FORWARD -i veth-host -s ${VETH_NS_IP} -d \${WG_ENDPOINT_IP} -j ACCEPT 2>/dev/null || true
iptables -A FORWARD -i veth-host -s ${VETH_NS_IP} -d \${WG_ENDPOINT_IP} -j ACCEPT

# Bring the tunnel up INSIDE the namespace - this becomes its only route out.
ip netns exec ${NS_NAME} wg-quick up wg0

# Route LAN return-traffic back through the host so the forwarded WebUI works (without this, replies to LAN clients would exit via the tunnel instead of
# coming back through the veth link).
ip netns exec ${NS_NAME} ip route replace ${LAN_SUBNET} via ${VETH_HOST_IP%/*} dev veth-ns

# One narrow port-forward so LAN devices can reach the WebUI at the machine's normal IP. Restricted to your LAN subnet as the source, on purpose.
...
#!/usr/bin/env bash
# Use the exact IP that up.sh resolved - this matches the rules it created
# and avoids relying on DNS during shutdown.
WG_ENDPOINT_IP=\$(cat /run/vpn-netns.endpoint 2>/dev/null || true)
if [[ -z "\${WG_ENDPOINT_IP}" ]] || [[ ! "\${WG_ENDPOINT_IP}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\$ ]]; then
  # Fallback (e.g. hand-run up.sh without /run): re-parse wg0.conf and resolve a hostname.
  WG_ENDPOINT_IP=\$(grep -iE '^\s*Endpoint\s*=' /etc/wireguard/wg0.conf | head -n1 | sed -E 's/^[[:space:]]*[Ee]ndpoint[[:space:]]*=[[:space:]]*//; s/[[:space:]]*$//' || true)
  WG_ENDPOINT_IP=\${WG_ENDPOINT_IP%:*}
  WG_ENDPOINT_IP=\${WG_ENDPOINT_IP#[}
  WG_ENDPOINT_IP=\${WG_ENDPOINT_IP%]}
  if [[ -n "\${WG_ENDPOINT_IP}" ]] && [[ ! "\${WG_ENDPOINT_IP}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\$ ]]; then
    WG_ENDPOINT_IP=\$(getent ahostsv4 "\${WG_ENDPOINT_IP}" | awk 'NR==1{print \$1}' || true)
  fi
fi
iptables -t nat -D POSTROUTING -s ${VETH_NS_IP} -d \${WG_ENDPOINT_IP} -j MASQUERADE 2>/dev/null || true
...
EOF
chmod 600 /opt/webhook/hooks.json
...
[Service]
# Lock the P2P switch + VPN status page down to the LAN on every start, so the rules
# also survive reboots (plain iptables rules are volatile). Runs as root BEFORE ExecStart.
# Note: this must live in a root service independent of vpn-netns - the webhook has to
# stay reachable to turn P2P ON/OFF while the namespace is stopped.
ExecStartPre=/bin/sh -c 'iptables -D INPUT -p tcp --dport ${WEBHOOK_PORT} -s ${LAN_SUBNET} -j ACCEPT 2>/dev/null; iptables -I INPUT 1 -p tcp --dport ${WEBHOOK_PORT} -s ${LAN_SUBNET} -j ACCEPT; iptables -D INPUT -p tcp --dport ${WEBHOOK_PORT} -j DROP 2>/dev/null; iptables -A INPUT -p tcp --dport ${WEBHOOK_PORT} -j DROP; iptables -D INPUT -p tcp --dport ${STATUS_PORT} -s ${LAN_SUBNET} -j ACCEPT 2>/dev/null; iptables -I INPUT 1 -p tcp --dport ${STATUS_PORT} -s ${LAN_SUBNET} -j ACCEPT; iptables -D INPUT -p tcp --dport ${STATUS_PORT} -j DROP 2>/dev/null; iptables -A INPUT -p tcp --dport ${STATUS_PORT} -j DROP'
...
[Unit]
Description=Refresh VPN egress status every 10 seconds
...
Type=simple
DynamicUser=yes
ExecStart=$(command -v python3) -m http.server ${STATUS_PORT} --bind 0.0.0.0 --directory /opt/vpn-netns/www
Restart=always
RestartSec=10
...
==================================================================
 ...
 VPN status widget (in Homarr: add an "Embed" widget with this URL):
   URL: http://${PI_IP}:${STATUS_PORT}        (raw JSON: http://${PI_IP}:${STATUS_PORT}/status.json)
...
[[ -z "${LEAK_CHECK_FAILED:-}" ]] || { echo "!! Setup finished, but the leak-check FAILED - the VPN tunnel is NOT confirmed working. Fix it before trusting this setup." >&2; exit 1; }